...
def retrieve_event_data(request):
    """ View to retrieve event data from the database and
        return it as a JSON response
    """
    try:
        events = Employee.objects.all()
        event_data = [{'name': event.name, 'date': event.email} for event in events]
        return Response({'events': event_data, 'message': 'Event data retrieved successfully.'})
            ...
            for attempt in range(max_attempts):
                ...
                try:
                    ...
                    if attempt == max_attempts - 1:
                        ...
                        EmailLog.objects.create(event=event, status='failed', error=str(ex))
            else:
                pass
...
def templates(request):
    ...
    template_ = EmailTemplate.objects.all()
    data = [{'event_type': template.event_type, 'template': template.template} for template in template_]
